"""
Spark NLP Pipeline Module for Automated ICD Diagnosis Extraction
"""
...
class SparkNLPPipeline:
    """
    Class for creating and managing Spark NLP pipeline for clinical NER
    """
    ...
    def extract_entities(self, df, input_col: str = "rekam_medis_narasi", 
                         output_col: str = "entities_detected") -> pd.DataFrame:
        """
        Extract entities from medical records DataFrame
        ...
        Returns:
            Pandas DataFrame with extracted entities
        """
        # Build pipeline
        ...
        results_pd['icd10_codes'] = results_pd[output_col].apply(
            lambda x: self.icd_mapper.map_terms_to_codes(x) if x is not None else []
        )
    ...
    def evaluate_results(self, results_df: pd.DataFrame,
        ...
        for _, row in results_df.iterrows():
            ...
            if ground_truth_value is None or (isinstance(ground_truth_value, float) and str(ground_truth_value) == 'nan'):
                ground_truth = ""
            else:
                ground_truth = str(ground_truth_value).lower()
...
def main():
    """
    Main function to demonstrate the Spark NLP Pipeline
    """
    # Setup logging
    ...
    # Initialize pipeline
    pipeline = SparkNLPPipeline()
    
    # Load sample data
    logger.info("Loading sample data...")
    try:
        df = pd.read_csv("../database/diagnosis_icd_2025.csv")
        spark_df = pipeline.spark.createDataFrame(df)
        
        # Extract entities
        logger.info("Extracting entities...")
        results = pipeline.extract_entities(spark_df)
        
        # Evaluate results
        logger.info("Evaluating extraction...")
        eval_metrics = pipeline.evaluate_results(results)
        ...
        output_file = f"../output/hasil_ekstraksi_entitas_{timestamp}.csv"
